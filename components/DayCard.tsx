import { InstructorColors } from "@/configs/classes"
...
const getColor = (instructors: string[]) => {
  if (instructors.length === 1) {
    return InstructorColors[instructors[0]]
  }
  return 'linear-gradient(90deg, ' + instructors.map(instructor => InstructorColors[instructor]).join(', ') + ')'
}
  ...
  return (
    <div className="grid gap-4 py-4" >
      {classes.map(({ name, time, instructors }, index) => (
        <div key={String(name + index)} style={{ background: getColor(instructors) }} className=" p-1 flex flex-col justify-center ">
